...
int checkKeys(XEvent *e)                                                                                 
{
    ...
    switch (key) {
        ...
        case XK_minus:
            gl.delay += 0.005;
            break;
        case XK_d: // Deal damage
            healthBar.setHealth(healthBar.getHealth() - 10.0f);
            break;
        case XK_h: // Heal
            healthBar.setHealth(healthBar.getHealth() + 10.0f);
            break;
        case XK_r: // Reset health if the game is over
            if (healthBar.isGameOver())
        {
            healthBar.setHealth(100.0f); // Reset health to maximum
         }
             break;
        case XK_Escape:
            return 1;
...
void render(void)
{
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    healthBar.draw(); //draw healthbar
    if (healthBar.isGameOver())
{   ggprint8b(&r, 16, 0xFF0000, "Game Over!");
    ggprint8b(&r, 16, 0xFFFFFF, "Press R to Restart or Q to Quit");
}
